...
export function BranchSwitcher() {
    ...
    if (!isOwner) {
        ...
        return (
            <div
                className="hidden md:flex items-center gap-1 bg-muted text-foreground px-2 py-1 rounded-full text-xs font-semibold border border-border"
                title={branchName ?? undefined}
            >
                <Building2 className="h-3.5 w-3.5 shrink-0" />
                <span className="truncate max-w-[90px]">{branchCode || branchName}</span>
            </div>
        );
    }
    ...
    if (branches.length === 1) {
        ...
        return (
            <div
                className="hidden md:flex items-center gap-1 bg-emerald-500/10 text-emerald-600 dark:text-emerald-300 px-2 py-1 rounded-full text-xs font-semibold border border-emerald-500/20"
                title={only.name}
            >
                <Building2 className="h-3.5 w-3.5 shrink-0" />
                <span className="truncate max-w-[90px]">{only.code || only.name}</span>
            </div>
        );
    }
    ...
    return (
        <div className="relative" ref={ref}>
            <button
                type="button"
                onClick={() => setOpen((v) => !v)}
                className="flex items-center gap-1 bg-emerald-500/10 text-emerald-600 dark:text-emerald-300 hover:bg-emerald-500/20 px-2 py-1 rounded-full text-xs font-semibold transition-colors border border-emerald-500/20"
                title={active ? active.name : "Pilih cabang aktif"}
            >
                {active ? (
                    <>
                        <Building2 className="h-3.5 w-3.5 shrink-0" />
                        <span className="truncate max-w-[72px]">{active.code || active.name}</span>
                    </>
                ) : (
                    <>
            ...
            {open && (
                <div className="absolute right-0 mt-2.5 w-64 rounded-2xl bg-card/70 backdrop-blur-2xl shadow-2xl ring-1 ring-black/5 border border-border/60 overflow-hidden z-50 animate-in fade-in slide-in-from-top-1 duration-150">
                    ...
                    <div className="max-h-72 overflow-y-auto py-1">
                        <button
                            ...
                            className={`flex w-full items-center gap-2.5 px-3 py-2 text-sm hover:bg-muted transition-colors ${
                                activeBranchId === null ? "bg-emerald-500/10" : ""
                            }`}
                        >
                        ...
                        {branches.length === 0 ? (
                            ...
                            branches.map((b) => (
                                <button
                                    ...
                                    className={`flex w-full items-center gap-2.5 px-3 py-2 text-sm hover:bg-muted transition-colors ${
                                        activeBranchId === b.id ? "bg-emerald-500/10" : ""
                                    }`}
                                >
