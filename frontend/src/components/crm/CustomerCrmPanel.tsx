...
export function CustomerCrmPanel({ customerId }: { customerId: number }) {
    ...
    if (isLoading) {
        return (
            <div className="border-t border-border pt-4 mt-4 flex items-center justify-center py-6 text-sm text-muted-foreground">
    ...
    return (
        <div className="border-t border-border pt-4 mt-4">
            <h3 className="font-bold text-base mb-3 flex items-center gap-2">
                <Sparkles className="h-5 w-5 text-primary" /> CRM
            </h3>

            {/* Assigned CS + Lead Source */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3 text-sm">
                <div>
                    <label className="block text-xs font-semibold text-muted-foreground mb-1">CS yang Pegang</label>
                    <select
                ...
                <div>
                    <label className="block text-xs font-semibold text-muted-foreground mb-1">Asal Lead</label>
                    <div className="px-3 py-2 border border-border rounded-lg bg-muted text-sm">
                        {c.leadSource || <span className="text-muted-foreground">Tidak ada data</span>}
                    </div>
                </div>
            </div>
            ...
            <div className="flex gap-2 flex-wrap mb-4">
                <button
                    ...
                    className="px-3 py-1.5 bg-primary text-primary-foreground rounded-lg text-xs font-semibold hover:bg-primary/90 flex items-center gap-1"
                >
                        ...
                        {data.followUps.map((fu: any) => (
                            <div
                                ...
                                className={`flex items-center gap-2 text-sm border rounded px-2 py-1.5 ${
                                    fu.status === "PENDING" ? "bg-amber-500/10 border-amber-500/20" : "bg-muted border-border opacity-70"
                                }`}
                            >
                                ...
                                {fu.notes && <span className="text-xs text-foreground truncate flex-1">— {fu.notes}</span>}
                                {fu.status === "PENDING" && (
                                    <button
            ...
            {/* Activity Timeline */}
            <div>
                <h4 className="font-semibold text-sm text-muted-foreground mb-2">Timeline Aktivitas</h4>
                {data.activities.length === 0 ? (
                    <p className="text-xs text-muted-foreground italic">Belum ada aktivitas CRM untuk customer ini.</p>
                ) : (
                    <div className="space-y-1.5">
                        {data.activities.slice(0, 10).map((a: any) => (
                            <div key={a.id} className="border-l-2 border-primary/30 pl-3 py-0.5">
                                ...
                                {a.text && <p className="text-xs text-foreground">{a.text}</p>}
                            </div>
                        ))}
                        {data.activities.length > 10 && (
                            <p className="text-xs text-muted-foreground italic">... dan {data.activities.length - 10} lagi</p>
                        )}
                    </div>
                )}
            </div>
...
function CreateFuModal({
    ...
    return (
        <div className="fixed inset-0 bg-background/25 backdrop-blur-md z-[400] flex items-center justify-center p-4">
            <div className="glass-strong rounded-xl p-5 max-w-md w-full">
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                    <div>
                        <label className="block text-xs font-semibold text-muted-foreground mb-1">Jenis</label>
                    ...
                    <div className="flex gap-2">
                        <button type="button" onClick={onClose} className="flex-1 px-4 py-2 border border-border rounded-lg text-sm">Batal</button>
...
function CustomerTemplateModal({
    ...
    return (
        <div className="fixed inset-0 bg-background/25 backdrop-blur-md z-[400] flex items-center justify-center p-4">
            <div className="glass-strong rounded-xl p-5 max-w-xl w-full max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3">
                    {(templates ?? []).map((t) => (
                        <button key={t.id} onClick={() => handlePick(t)}
                            className={`text-left p-2 border rounded-lg hover:border-primary/40 ${
                                selected?.id === t.id ? "border-primary bg-primary/10" : "border-border"
                            }`}>
                            <div className="text-[10px] text-primary font-semibold">{t.category}</div>
