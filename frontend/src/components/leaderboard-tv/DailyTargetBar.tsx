...
export default function DailyTargetBar({ data, branchId }: { data: DT | null; branchId: number | null }) {
  ...
  return (
    <div className="px-4 py-2.5 border-b border-border bg-card/50 backdrop-blur">
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2 shrink-0">
          <Target className={`w-6 h-6 ${met ? 'text-emerald-500' : 'text-amber-500'} ${met ? '' : 'tv-float'}`} />
          <span className="text-base font-black uppercase tracking-wide">Target Harian</span>
        </div>

        {/* Bar progres beranimasi */}
        <div className="relative flex-1 h-8 rounded-full bg-muted overflow-hidden border border-border">
          <div
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-sm font-black text-foreground drop-shadow tabular-nums">
      ...
      {/* Chip per-cabang saat mode semua cabang */}
      {branchId == null && withTarget.length > 1 && (
        <div className="flex flex-wrap gap-2 mt-2">
          {withTarget.map(b => (
            <span key={b.branchId}
              className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${b.met ? 'border-emerald-500/40 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'border-border bg-background text-muted-foreground'}`}>
              <span className={`w-2 h-2 rounded-full ${b.met ? 'bg-emerald-500' : b.pct >= 60 ? 'bg-amber-500' : 'bg-rose-500'} ${b.met ? '' : 'animate-pulse'}`} />
              {b.branchName}
