        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Template WhatsApp untuk follow-up, greeting, after-sales — dipakai manual (copy & paste)</p>
                </div>
...
function TemplateCard({ template, onEdit, onDelete, onPreview, onToggleActive }: {
    ...
    return (
        <div className={`bg-card border rounded-xl p-4 ${!template.isActive ? "opacity-60" : ""}`}>
            <div className="flex items-start justify-between mb-2">
                <div className="font-bold text-foreground flex-1 truncate">{template.name}</div>
                {!template.isActive && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground font-semibold">NONAKTIF</span>
                )}
            </div>
        ...
        <div className="fixed inset-0 bg-background/25 backdrop-blur-md z-[200] flex items-center justify-center p-4">
            <div className="bg-card rounded-xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-3">
                    <div>
                        <label className="block text-xs font-semibold text-muted-foreground mb-1">Nama Template *</label>
                        <input
                            required
                            value={form.name}
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-muted-foreground mb-1">Body Template *</label>
                        <textarea
                            required
                            rows={8}
