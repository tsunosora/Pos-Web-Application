...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: any }> = {
    PAID:    { label: "Lunas",         color: "bg-emerald-500/10 text-emerald-600 dark:text-emerald-300 border border-emerald-500/20", icon: CheckCircle },
    PARTIAL: { label: "DP / Sebagian", color: "bg-amber-500/10 text-amber-600 dark:text-amber-300 border border-amber-500/20",       icon: Clock },
    PENDING: { label: "Belum Bayar",   color: "bg-muted text-muted-foreground border border-border",                                  icon: Clock },
    FAILED:  { label: "Dibatalkan",    color: "bg-red-500/10 text-red-600 dark:text-red-300 border border-red-500/20",               icon: AlertCircle },
};
        ...
        return (
            <div className="space-y-4">
                ...
                <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-6 text-center">
                    <p className="text-red-600 dark:text-red-300 font-medium">Transaksi tidak ditemukan</p>
                </div>
            </div>
        );
    }

    const statusCfg = STATUS_CONFIG[trx.status] ?? { label: trx.status, color: "bg-muted text-muted-foreground border border-border", icon: AlertCircle };
            ...
            <div className="print:hidden space-y-6 max-w-4xl">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div>
                            ...
                            {trx.productionBranchId != null && trx.productionBranchId !== trx.branchId && (
                                <div className="mt-1.5 inline-flex items-center gap-1.5 px-2.5 py-1 bg-amber-500/10 text-amber-600 dark:text-amber-300 border border-amber-500/20 rounded-full text-xs font-bold"
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-4">
                        <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden transition-[box-shadow,border-color] hover:shadow-md hover:border-primary/40">
                                        ...
                                        return (
                                            <tr key={item.id} className="hover:bg-accent transition-colors">
                                                <td className="px-4 py-3">
                                                    ...
                                                    {item.clickType && (
                                                        <p className="text-xs text-violet-600 dark:text-violet-300 flex items-center gap-1">
                    ...
                    <div className="space-y-4">
                        {/* Customer info */}
                        <div className="bg-card rounded-xl border border-border shadow-sm p-4 space-y-3 transition-[box-shadow,border-color] hover:shadow-md hover:border-primary/40">
                        ...
                        {trx.printJobs && trx.printJobs.length > 0 && (
                            <div className="bg-card rounded-xl border border-border shadow-sm p-4 space-y-2 transition-[box-shadow,border-color] hover:shadow-md hover:border-primary/40">
                                ...
                                {trx.printJobs.map(pj => {
                                    ...
                                    const statusMap: Record<string, string> = {
                                        ...
                                        PROSES: 'bg-indigo-500/10 text-indigo-600 dark:text-indigo-300 border-indigo-500/20',
                                        SELESAI: 'bg-green-500/10 text-green-600 dark:text-green-300 border-green-500/20',
                                        DIAMBIL: 'bg-blue-500/10 text-blue-600 dark:text-blue-300 border-blue-500/20',
                                    };
                                    const lastTs = pj.pickedUpAt || pj.finishedAt || pj.startedAt;
                                    return (
                                        <div key={pj.id} className="text-xs border-t border-border/60 first:border-t-0 pt-2 first:pt-0">
                                ...
                                {marketplaceFee > 0 && (
                                    <>
                                        {(trx.marketplaceFeeItems && trx.marketplaceFeeItems.length > 0
                                            ...
                                            <div key={i} className="flex justify-between text-red-600 dark:text-red-300 text-sm">
                                                <span>{fi.name}</span>
                                                <span>− {formatRp(Number(fi.amount))}</span>
                                            </div>
                                        ))}
                                        {trx.marketplaceFeeItems && trx.marketplaceFeeItems.length > 1 && (
                                            <div className="flex justify-between text-red-600 dark:text-red-300 font-semibold text-sm border-t border-red-500/20 pt-1">
                                                <span>Total Potongan</span>
                                                <span>− {formatRp(marketplaceFee)}</span>
                                            </div>
                                        )}
                                        <div className="flex justify-between text-emerald-600 dark:text-emerald-300 font-semibold text-sm">
