                    ...
                    <?php foreach ($items as $it): $sub = cart_item_subtotal($it); $isAreaItem = !empty($it['widthCm']) && !empty($it['heightCm']); ?>
                        <tr>
                            <td class="px-6 py-3 text-slate-800">
                                ...
                                <?php if ($isAreaItem): $ut = $it['unitType'] ?? 'cm'; $ul = $ut === 'm' ? 'm' : ($ut === 'menit' ? 'unit' : 'cm'); $sq = area_sq_label($ut); ?><span class="block text-xs text-slate-400"><?= h($it['widthCm']) ?>&times;<?= h($it['heightCm']) ?> <?= h($ul) ?> (<?= h(rtrim(rtrim(number_format(area_native((float)$it['widthCm'], (float)$it['heightCm'], $ut), 2, ',', '.'), '0'), ',')) ?> <?= h($sq) ?>)</span><?php endif; ?>
                            </td>
                            <td class="px-6 py-3 text-center text-slate-600"><?= (int)($it['quantity'] ?? 0) ?></td>
                            <td class="px-6 py-3 text-right text-slate-600"><?= rupiah($it['unitPrice'] ?? 0) ?><?= $isAreaItem ? '<span class="text-xs text-slate-400">/' . h(area_sq_label($it['unitType'] ?? 'cm')) . '</span>' : '' ?></td>
